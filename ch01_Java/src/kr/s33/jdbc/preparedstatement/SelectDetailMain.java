...
public class SelectDetailMain {
	public static void main(String[] args) {
		...
		try {
			...
			//?에 데이터 바인딩
			pstmt.setInt(1, num);//여기에 담긴게 위로
			
			//JDBC 수행 4단계 : SQL문을 테이블에 반영하고 결과집합을 ResultSet에 담아서 반환
			rs = pstmt.executeQuery();
			if(rs.next()) {//프라이머리키로 하나의 데이터를 읽어올때는 if
				System.out.println("번호 : " + rs.getInt("num"));//내가입력한 num값이어서 ""붙여야됨
				...
				//System.out.println("이메일 : " + rs.getString("email")); 위에if조건 없으면 null인상태로 출력됨
				System.out.println("등록일 : " + rs.getDate("reg_date"));
			}else {
				System.out.println("검색된 데이터가 없습니다.");//프라이머리 잘못입력
			}
		}catch(Exception e) {
			e.printStackTrace();
		}finally {
