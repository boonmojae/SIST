$(function(){
	...
	$('.cart-modify').on('click',function(){
		const input_quantity = $(this).parent().find('input[name="order_quantity"]');//this는 변경 버튼
		if(input_quantity.val()==''){
			alert('수량을 입력하세요');
			input_quantity.focus();
			return;
		}
		if(input_quantity.val()<1){//변경된 값
			alert('상품의 최소 수량은 1입니다');
			//태그에 명시한 원래 value 값을 읽어옴
			input_quantity.val(input_quantity.attr('value'));//attr속성으로 value호출하면 원래 값을 읽어옴
			return;
		}
		//서버와 통신
		$.ajax({
