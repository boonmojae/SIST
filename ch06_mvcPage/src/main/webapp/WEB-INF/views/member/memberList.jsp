...
<div class="page-main">
	...
	<div class="content-main">
		<h2>회원목록(관리자 전용)</h2>
		<form id="search_form" action="adminList.do" method="get"><%--주소가 안보이게 post방식으로 하는데 이거는 일반적으로 get방식으로 전달 --%>
			<ul class="search">
				<li>
					<select name="keyfield">
						<option value="1" <c:if test="${param.keyfield==1}">selected</c:if>>아이디</option><%-- keyfield는 아이디,이름,이메일인지 식별 역할(검색범위 지정) --%>
		...
		<div class="list-space align-right">
			...
			<input type="button" value="홈으로" onclick="location.href='${pageContext.request.contextPath}/main/main.do'"><%--경로가 달라서 uri--%>
		</div>
		<c:if test="${count==0}">
