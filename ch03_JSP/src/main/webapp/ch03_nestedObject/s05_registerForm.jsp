	...
	window.onload=function(){
		...
		myForm.onsubmit = function(){
			...
			for(let i=0;i<items.length;i++){
				...
				//정규표현식
				if(items[i].id == 'id' && !/^[A-Za-z0-9]{4,12}$/.test(items[i].value)){//id인것만 일치하면 밑에있는 내용 처리/!꼭 써야됨
					alert('영문 또는 숫자 사용, 최소 4자 ~ 최대 12자 사용');
					items[i].value='';
					items[i].focus();
					return false;//패턴에 안맞는거 걸러냄
				}
			}
...
<form action="s06_register.jsp" method="post" id="myForm">
	<ul>
		...
		<li>
			...
			<textarea rows="3" cols="22" name="content"></textarea>
		</li>
	</ul>
