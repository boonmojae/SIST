...
create table spmember_detail(
 ...
 photo blob, --바이너리 데이터를 blob로(프로필 사진만 테이블에)
 photo_name varchar2(100), --파일명 생략 가능,파일명은 바꿀수 있기때문에 그냥 옵션
...
create table spboard(
	...
	constraint spboard_kf foreign key (mem_num) references spmember(mem_num)
);
...
create table spboard_response(
 ...
 te_mdate date,
 te_parent_num number not null, -- 부모글의 번호가 들어감, 자식글이 아니라 부모글일 경우 0(부모일경우 0으로 들어가 foreign key를 줄수 없음)
 te_depth number not null, --자식글의 깊이. 부모글의 자식글 A 1,  자식글A의 자식글B 2, 부모글일 경우 0
