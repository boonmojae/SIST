...
public class CommonController {
	...
	public Map<String, Object> uploadImage(MultipartFile upload,HttpSession session,HttpServletRequest request,HttpServletResponse response)throws Exception{
		...
		//업로드할 폴더 경로(절대 경로)/파일처리할땐 절대 경로가 필요
		...
		//항상 로그인 되어있는 상태가 아닐때 가정
		if(user == null) {
			sub_path = "general";
		}else {
			sub_path = String.valueOf(user.getMem_num());//로그인이 되어있다면 해당 회원이 올린 이미지가 다 갖은곳에 들어가게
		}
		String file_path = realFolder + "/" + sub_path + "/" + str_filename;//파일명 가공
		
		log.debug("<<파일 경로>> : " + file_path);
		File f = new File(file_path);
		if(!f.exists()) {//상위경로가 없을때
			//폴더 만든다음에 파일 업로드
			//상위 경로를 생성해야 하기 때문에 mkdirs()
			//mkdir()를 상용하면 상위 경로를 생성하지 못함
			f.mkdirs();
		}
		//상위경로 있을때
		upload.transferTo(f);
		...
		map.put("uploaded",true);//ckeditor에서 정해놓은거로 이렇게 해야됨
